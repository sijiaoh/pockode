...
| Migrating `web`'s two remaining hand-rolled body-scroll locks (`web/src/components/Files/UploadConflictDialog.tsx`, `web/src/components/ui/ResponsivePanel.tsx`) to `useLockBodyScroll` | It is pure `web` work, not cluster work. Correct today only because neither has an overlay it can stack with — which is fragile, not safe. `ResponsivePanel`'s also keeps it out of `useIsPageCovered`, which is part of why Escape over it still interrupts the agent's turn ([answering-ui.md](answering-ui.md#who-owns-escape)). |
