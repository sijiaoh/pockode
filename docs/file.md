...
inflates it by 4/3, and over a relay tunnel that one write holds the shared
connection lock until it drains — which is the reason for the 2 MiB ceiling
above. Transferred files are routinely larger than that, so putting them on the
same connection would stall every other stream and outlive the keepalive (see
[code/relay-system.md](code/relay-system.md#write-lock-protection)). HTTP keeps
them off it, lets the endpoint stream both directions rather than hold a file in
memory, and is what the browser already speaks for files: `Content-Disposition`
...
leaves open. The relay's side of the ceiling is derived in
[code/relay-system.md](code/relay-system.md#size-ceiling), and why it has to be
answered per connection is in
[code/websocket-rpc.md](code/websocket-rpc.md#connection-route).
...
**A relay connection carries a smaller limit.** The tunnel puts a whole HTTP
request inside one base64-encoded WebSocket envelope and refuses to read an
envelope past a fixed size, so an upload arriving that way is bounded by the
tunnel long before `MaxUploadSize` — and crossing that bound is not a `413`.
The request never arrives as a request: the read limit is overrun and the
tunnel drops, reaching the user as an unexplained reconnect. Nothing is left
for the endpoint to refuse, so saying the true number in advance is the only
defence there is, and that is what `max_upload_size` carries. It describes
**the route the connection came in on**: `RouteRelay` gets what fits in the
tunnel, `RouteDirect` gets `MaxUploadSize`, and one server answers both at once
(see [code/websocket-rpc.md](code/websocket-rpc.md#connection-route)). The value
is derived from the envelope ceiling rather than written down
([code/relay-system.md](code/relay-system.md#size-ceiling) has the arithmetic);
it currently lands on **7 MiB** and follows that ceiling if it ever moves.

A download response is enveloped the same way and yet is quoted no ceiling at
all, because asking for bounded pieces removes the need for one — which is why
the client chunks unconditionally (see [Downloading](#downloading)). An upload
cannot answer it that way: one request is indivisible.
...
**Reads are always chunked**, 4 MiB per `Range` request, even on a direct
connection that has no need of it. The browser cannot tell which of the two it
is on, and over a relay tunnel an unchunked response is read into memory whole
and base64-inflated past the envelope ceiling, so the safe path is the only
path; one extra round trip per 4 MiB is what it costs locally.
...
exists to prevent, so the mechanism guarding the download would be the thing
that drops the tunnel. `Last-Modified` is a weak validator (one-second
...
Three gaps rather than decisions, collected here because each of them is easy to
rediscover as a bug:

- **The relay does not stream bodies.** It buffers a whole request or response
  into one envelope, which is what caps a remote upload at 7 MiB instead of the
  endpoint's 32 MiB. Forwarding a body as it arrives is the real fix and would
  lift the limit back to `MaxUploadSize`; raising the envelope ceiling instead
  would only move the failure, and the other end of the tunnel is the cloud
  relay, outside this repository. Until then a remote client is at least told
  the truth about what it can send — as far as this side can know it. A
  stricter limit at the cloud end would fail identically and could only be
  found by observation, since 7 MiB is derived from what *this* side reads.
