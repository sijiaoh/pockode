...
action — 36px of visual weight, borderless, muted until hovered, grown to a 44px
hit area where a finger may land
...
| `common/MenuRow.tsx` | `MenuRow` + `menuRowClass`, **extracted from `Files/FileEntryMenu.tsx`**, which switches to the shared one. Two sheets of menu rows is where that stops being one component's private detail |
