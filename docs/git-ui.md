...
- **The panel is narrow.** `DiffTab` renders inside the tabbed sidebar: a `w-72` (288px) overlay drawer below `expanded` (1024), a 240–500px resizable standing column at and above it ([responsive-ui.md](responsive-ui.md#the-width-ladder)). Every layout below is drawn at 288px, and nothing may rely on width the panel does not have.
- **One-handed and touch-first.** Full-width rows are `min-h-[44px]`; icon-only buttons inside a row are `min-h-[36px] min-w-[36px]` (L5), defined once in `ui/iconButtonClass.ts` so stage, unstage, discard, amend and dismiss cannot drift apart in weight. 36 is the visual rung; a coarse pointer grows the box to 44 ([responsive-ui.md](responsive-ui.md#hit-areas-and-spacing)).
...
- **Reuse the existing vocabulary.** `th-*` tokens, `ConfirmDialog` from `@pockode/shared`, `BottomActionBar`, `SidebarListItem`, lucide icons, and the bottom-drawer-below-`expanded` / centred-modal-above pattern already used by `WorktreeCreateSheet` (extracted as `Sheet`, since promoted to `@pockode/shared`, which the four sheets below share).
...
├──────────────────────────────────┤
│ ⚠ Discard failed · Details     ✕ │  error banner — fixed, after a failure; Details only if git ran
├──────────────────────────────────┤
│ STAGED 2                     ⊖   │  L3, sticky   ┐
│  M  DiffTab.tsx              ⊖   │               │
│  A  BranchBar.tsx            ⊖   │               │
...
**A running sync adds a spinner in front of whichever of those states the chip is showing**, and the accessible name becomes `Sync with remote, pulling…`. It leads rather than replaces: the `Publish` state has no icon to take over, and the counts are still the truth until the operation refreshes them, so nothing already on the chip is swapped out or blanked. The spinner stops when the run settles, and what replaces it is the refreshed count — the success signal arrives where the user is already looking, which is why success needs no message of its own ([Feedback and errors](#feedback-and-errors)).

The run it shows is the one thing on this chip that is **not** derived from the server's answer; it comes from `lib/gitSyncStore.ts`, and may have been started from a sheet the user has since closed ([Remote sync](#remote-sync)). That extra input is accepted deliberately: the sync sheet is this chip expanded, so the chip is where the action was started from, and sync has no other permanent place in the panel to report from.
...
**The sheet used to be unusable with many branches, and the cause was in `Sheet`, not here.** The content box capped its height in its drawer form (`max-h-[90dvh]`) and not in its modal form (`mx-4 max-w-md rounded-xl`). Centred inside a `fixed inset-0` container, it spilled off the top and the bottom at once as soon as the list made it taller than the viewport, and nothing scrolled: the body is `min-h-0 flex-1 overflow-y-auto`, but `flex-1` of an uncapped column is the content's own height, so there was never any overflow to scroll — the `New branch…` footer went off-screen with the rest. The modal form now caps at `max-h-[85dvh]` too. That is a fix in `Sheet`, so every one of its users stops being able to reach that state — the four sheets in this document plus `Worktree/WorktreeCreateSheet`.
...
- These are the only operations in the panel that wait on a machine other than this one. While one runs, its button shows a spinner and the gerund (`Pushing…`) and the others are disabled — but **the sheet can still be closed.** Holding a sheet shut is a way of saying "this is over in a moment", and it only tells the truth where the wait is bounded; these three are network round trips whose only ceiling is the backend's five-minute `networkTimeout` ([git.md](git.md#remote-sync)), so holding this one shut would be a promise the panel cannot keep.

  **Closing is leaving, not cancelling.** The operation keeps running, the chip carries its progress, a failure comes back under the branch bar, and reopening the sheet shows *that same run* — the running button still spinning, or the outcome it ended in — rather than a blank panel offering to start it again. That works because the run, its result and the guard against a second one all live in `lib/gitSyncStore.ts` rather than in the sheet; the sheet is a renderer of state it does not own. Nothing here cancels git: a cancelled push is not a push that did not happen, and the panel does not offer an action it cannot honestly undo.

  **Leaving leaves the rest of the panel usable, and mostly it really is usable.** Staging, discarding and committing while a fetch or a push runs all go through — not because nothing arbitrates them, but because the server's arbitration is split by what git commands actually contend for, and staging conflicts with a fetch in no way at all ([git.md](git.md#serialising-writes)). Nothing in the UI holds them back either: the sheet no longer covers the panel, and the sync store's guard stops only a *second sync*.

  What a running fetch or push does hold up is the writes that move the same refs it does: **pull, switching branch and creating one**. A running *pull* holds up everything, because it is the one sync that fast-forwards the working tree as well. Either way the second write waits briefly and, since the sync is usually still going, is refused before it starts. The refusal is not a race lost to `index.lock` — the second command never ran — so what comes back is a sentence naming what to wait for rather than git's words about a lock file: *This worktree is busy fetching. Try again once it finishes.* ([Feedback and errors](#feedback-and-errors))

  The sheet *is* held while the force-push confirmation is up: `Sheet` and `ConfirmDialog` both listen for Escape on `document`, so a single key press would otherwise close the dialog and the sheet behind it. And it stays open after success long enough to show the outcome (`Pushed 1 commit.`): a sheet that closes on its own leaves the user unsure whether anything happened over a slow relay link.
...
The project has no toast system — `WorktreeSwitcher` still carries a `TODO` about it — and the panel does not introduce one. The rule instead is **feedback appears where the action started** — and where the starting place can be left mid-action, the panel itself carries what the action leaves behind:

- **Actions started in a sheet the user cannot leave** (commit, switch, create) render their error inside that sheet, in the `text-th-error` + `role="alert"` form `WorktreeCreateSheet` already uses. The sheet stays open and the user's typed input survives.
- **Fetch, pull and push** are started in a sheet that *can* be closed while they run ([Remote sync](#remote-sync)), so one outcome is rendered in whichever place is on screen. Sheet open: the same in-sheet `role="alert"` block as above, unchanged. Sheet closed: the **error banner** under the branch bar, below. There is one outcome either way, not two copies of it — opening the sheet moves the message into it and takes the banner away, closing it brings the banner back, and there is no "already seen" state in between. A success is not banner material at all: its signal is the chip's spinner stopping and the counts refreshing, and it waits in the sheet as `Already up to date.` or `Pushed 2 commits.` for whoever reopens it.
- **Actions started inline from a row** — stage, unstage, discard, discard all — have no sheet to hold the result, so they use that same dismissible **error banner**: one line of plain-language summary, git's stderr behind a `Details` disclosure in a mono block, and a `✕`. It covers stage and unstage rather than discard alone, and not for symmetry: a failed stage used to reach nothing on screen at all — no surface rendered the mutation's error and the panel's own handler let the rejection escape — which is precisely the silent failure the project forbids. The banner clears on dismissal, or when a later attempt succeeds on one of the paths that failed. An unrelated file succeeding is not a retry: dropping the message then would leave the file that failed sitting in the list with nothing on screen explaining why.
- **The banner holds at most one failure *per source*** — sync above, from `BranchBar`, and the inline row actions below it, from `DiffTab` — each with its own `✕`. Two sources can be unacknowledged at once and both are real; collapsing them to "the most recent failure" would silently drop one. Within a source there is only ever one: a new run replaces the outcome of the last. `DiffView` carries a banner of its own, because its stage button is in the content area with the panel nowhere in sight.
- **A refused request has no `Details`, because git never ran.** The server refuses a write while another one it would collide with holds the worktree — which pairs those are is the server's to decide and is decided narrowly, a fetch never standing in the way of staging a file ([git.md](git.md#serialising-writes)). That refusal is a fact about the panel's own traffic rather than anything git said — quoting the server's English sentence back under a summary of itself would be the panel talking to itself. So `utils/gitErrors.ts` is the one place the two kinds of failure are told apart, by error code and never by matching the sentence, and every surface renders the outcome: a summary always, git's words only when there are any. The sentence names the operation to wait for in the panel's own vocabulary (`switching branches`, not `checkout`), and falls back to "another git operation" for an operation name it does not know, so a write added on the server is still explained rather than mis-explained. The branch sheet is the one place that embeds the sentence in its own — `Could not switch to <branch>. This worktree is busy pushing.` — because the branch list is taller than the sheet and the row that was tapped is usually scrolled away, while the refusal by itself cannot say which row it was.
- **The panel queues its own fast writes rather than leaning on that refusal.** Stage, unstage and discard run one at a time per worktree through `lib/gitWriteStore.ts`: tapping two files' stage buttons in a row is two `git.add` requests, each row disables only *itself* while its own request is in flight, and react-query does not serialise mutations — so without the queue the commonest interaction in the panel would be answered with *This worktree is busy staging files*, an error the user did nothing to earn. They **queue** rather than being dropped, unlike the sync store's guard: the second tap names a different file, so it is a second thing the user asked for and not a duplicate of the first. Requests are not merged into one batch either, though the server would take it: merging means either a debounce, which makes a single tap slower for nothing, or bookkeeping that re-derives which row's spinner and which banner a combined failure belongs to. Only the fast writes are queued — commit runs hooks and the three sync operations wait on another machine, so queueing behind one of those is the hang the server's bounded wait exists to prevent, and each of them is behind a single control that cannot be double-tapped into a collision.

  **A queued write is dropped if the user leaves the worktree before its turn.** A `git.*` request names no worktree: the server answers it against whichever one the connection is bound to, and `worktree.switch` rebinds it. Sending each write immediately used to make that safe by itself, since one connection is answered in order — a queue is what gives a switch room to land in between, so the queue is what has to check, at the moment the request would go out. Nothing is shown when one is dropped: the write never reached git, and a banner about a file in the tree the user has just left is the thing this panel already refuses to carry across a switch. The uploads queue cancels for the same reason, and does say so, because its queue stays on screen per worktree.
- **The sync banner yields to the sheet only when the sheet is really on screen**, not when it is merely the selected one: without branch data there is no sheet to render, and a banner suppressed by a sheet that is not there is a silent failure. For the same reason it is not gated on the chip either — a run that ends after HEAD went detached, or after `git.branches` started failing, has lost the chip that was carrying its progress, and losing progress is acceptable where losing a failure is not. Nor does leaving the Git tab lose it: the outcome is stored state rather than a notification, so it is still under the branch bar when the user comes back, which is exactly what a toast would not have managed.
...
**Refusing to close the surface is not one of those uniform rules.** It applies only to bounded operations, which is why the sync sheet no longer does it ([Remote sync](#remote-sync)). The commit, branch and new-branch sheets still do — and for commit that is not because committing is always quick, since a `pre-commit` hook can take as long as it likes. It is because those three hold typed input: closing them would have to answer "is what I wrote still there", and a commit message is the one thing in the panel the user cannot get back. The sync sheet holds no input and only has to answer "where did the result go" — which the chip and the banner answer.
...
| `ui/iconButtonClass.ts` | The L5 icon button's classes, used by `DiffFileItem`, `DiffFileList`, `LogList` and `ErrorBanner` here, by the file tree row's `…` in the Files panel, and by the `…` beside every chat bubble ([session-fork-ui.md](session-fork-ui.md)), which is also the one caller that asks for the overlay instead of a grown box. It used to live under `Git/` while the tree wrote its own copy, because the two rungs disagreed on height — 36 against 44 — and parameterising that would have meant sharing everything about the rung except the part that made it a rung. The disagreement turned out to be one rung measured two ways: 36 of visual weight, 44 of hit area on a coarse pointer ([responsive-ui.md](responsive-ui.md#hit-areas-and-spacing)). With both stated here, there is one definition and it belongs to no single panel |
| `Git/GitOutput.tsx` | The capped, scrollable block holding git's own words, shared by the error banner and the commit, branch and sync sheets. Every one of them renders it conditionally: a refused write has no git output to hold ([above](#feedback-and-errors)) |
...
| `lib/gitSyncStore.ts` | Which fetch/pull/push is in flight in each worktree and how the last one ended, so a run outlives the sheet that started it. Keyed by worktree because switching worktrees mid-run is reachable, and one record would show A's outcome under B or let A's run disable B's buttons |
| `lib/gitWriteStore.ts` | One serial queue per worktree for stage, unstage and discard, plus the paths each has pending, so two taps in a row cannot overlap and a write outlives the view that started it. Keyed by worktree for `gitSyncStore`'s reason |
| `hooks/useGitWrites.ts` | `useGitWriteRunner`, the single way into those three writes — the file list and the open diff both go through it, which is what makes one queue cover them together |
| `utils/gitErrors.ts` | The one place "git never ran" and "git ran and failed" are told apart, and the sentence a refusal is shown as — pure functions, so the wording and the code-not-prose rule are testable without a panel around them |
| `utils/gitSyncMessages.ts` | Every sentence the three sync operations produce, and the patterns that tell git's rejections apart — pure functions, so the wording is testable without a sheet around it |

The writes are `git.add` and `git.reset`, which the panel already had, plus `git.discard`, `git.commit`, `git.checkout`, `git.branch.create`, `git.fetch`, `git.pull` and `git.push`. A failure that git produced comes back as git's own words — stderr for most of them, stdout as well for commit, which is where git puts `nothing to commit` — and that text is what both surfaces above show under their summary; a write refused before it started has no such text ([above](#feedback-and-errors)). Shapes, and the server-side reasoning behind them, are in [git.md](git.md).
