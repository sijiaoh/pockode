...
- These are the only operations in the panel that wait on a machine other than this one. While one runs, its button shows a spinner and the gerund (`Pushing…`) and the others are disabled — but **the sheet can still be closed.** Holding a sheet shut is a way of saying "this is over in a moment", and it only tells the truth where the wait is bounded; these three are network round trips whose only ceiling is the backend's five-minute `networkTimeout` ([git.md](git.md#remote-sync)), so holding this one shut would be a promise the panel cannot keep.
  ...
  **Leaving leaves the rest of the panel usable, and nothing holds it back.** Staging, discarding, committing or switching branch while a fetch, pull or push runs is reachable now that the sheet no longer covers the panel, and the sync store's guard stops only a *second sync*. Two git commands in one worktree are serialised nowhere: whichever loses the race for `index.lock` fails, and git's refusal is shown verbatim like any other. That is visible rather than silent, which is the bar this panel holds itself to — but making it impossible needs a per-worktree lock on the server, which does not exist.
...
- **Actions started inline** (discard, discard all) have no sheet to hold the result, so they use that same dismissible **error banner**: one line of plain-language summary, a `Details` disclosure revealing git's stderr in a mono block, and a `✕`. The discard banner clears on dismissal or when a later discard succeeds on one of the paths that failed. An unrelated file succeeding is not a retry: dropping the message then would leave the file that failed sitting in the list with nothing on screen explaining why.
- **The banner holds at most one failure *per source*** — sync above, from `BranchBar`, and discard below it, from `DiffTab` — each with its own `✕`. Two sources can be unacknowledged at once and both are real; collapsing them to "the most recent failure" would silently drop one. Within a source there is only ever one: a new run replaces the outcome of the last.
