...
- **One-handed and touch-first.** Full-width rows are `min-h-[44px]`; icon-only buttons inside a row keep the existing `min-h-[36px] min-w-[36px]` of `DiffFileItem`.
- **Don't rewrite the information architecture.** Changes / History stay exactly as they are. Everything new attaches to the panel frame (a bar above, a bar below) or to rows that already exist.
...
`DiffTab`'s root is a flex column with `overflow-hidden`, and `PullToRefresh` inside it is `flex-1` with its own scroll. That gives three bands: a fixed header, the existing scroll area, and a fixed footer, with the new bars as siblings of `PullToRefresh` rather than content inside it.

```
┌──────────────────────────────────┐
│ ⑂ feature/git-ui        [↓2 ↑1]  │  branch bar   — fixed
├──────────────────────────────────┤
...
│    A  BranchBar.tsx           ⊖  │  │
│                                  │  │
│    UNSTAGED (3)     ↶  Stage All │  ├ existing scroll area
│    M  git.md            ↶     ⊕  │  │
│    ?  notes.txt         ↶     ⊕  │  │
│                                  │  │
│ ▾ History                        │  │
│    Render every codex_changes…   │  │
│    9692fc9 sijiaoh, 2h ago       │  ┘
├──────────────────────────────────┤
│ [          Commit (2)          ] │  commit bar   — fixed
└──────────────────────────────────┘
```

Both new bars sit outside the scroll area, so branch identity and the commit action stay reachable no matter how far the user has scrolled into History. They also render outside the current `isLoading` / `error` branch: a failing `git.status` must not take the branch bar down with it.
...
The bottom bar holds exactly one primary button, adapting to what is actually committable:

| Situation | Label | Enabled |
|-----------|-------|---------|
| n files staged | `Commit (n)` | yes |
| nothing staged, HEAD exists | `Amend last commit` | yes — opens the sheet with Amend pre-toggled |
| nothing staged, no commits yet | `Commit` | no |
| `git.status` unreadable | `Commit` | no — nothing is known about what is committable, and offering to amend would be a guess |
...
- **All unstaged** — an `Undo2` icon button in the `UNSTAGED (n)` group header, separated by a gap from `Stage All` so the two are not flush neighbours.
...
Three stacked full-width buttons, each `min-h-[44px]`. The counts are restated in words above them because `↓2 ↑1` is a chip abbreviation, not an explanation.
...
**Why this file lives here.** `docs/` holds per-feature design documents (`file.md`, `agent-chat.md`, `git.md`); `docs/code/` is reserved for explaining why the code of a *core module* is built the way it is (WebSocket, Agent, Work, Subscription, Relay), and `docs/projects/` for the project-management system. A UI design for a feature panel is none of those, so it sits beside `git.md` as `git-ui.md` — `git.md` keeps describing the git backend and links here for the surface built on top of it.
