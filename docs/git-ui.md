...
- **Reuse the existing vocabulary.** `th-*` tokens, `ConfirmDialog` from `@pockode/shared`, `BottomActionBar`, `SidebarListItem`, lucide icons, and the bottom-drawer-below-`expanded` / centred-modal-above pattern already used by `WorktreeCreateSheet` (extracted as `ui/Sheet`, which the four sheets below share).
