...
| `Git/iconButtonClass.ts` | The L5 icon button's classes, shared by `DiffFileItem`, `DiffFileList`, `LogList` and `ErrorBanner`. It stays under `Git/`: the Files panel's L5 buttons are round and float over the tree rather than sitting square inside a row, so promoting this to `components/common/` would mean unifying two treatments that differ on purpose, not sharing one |
