...
| `common/iconButtonClass.ts` | The L5 icon button's classes, used here by `DiffFileItem`, `DiffFileList`, `LogList` and `ErrorBanner`. It left `Git/` when a second panel took the same rung at the same height — the `…` under every chat bubble ([session-fork-ui.md](session-fork-ui.md)) — which is what settled that the 36px floor is not a Git fact. The Files panel's square in-row L5, the tree row's `…`, still writes its own copy instead of importing it: it takes that floor at 44, so sharing would mean parameterising the one thing this file exists to hold fixed. That divergence is not a Git-side decision and is flagged for resolution in [sidebar-ui.md](sidebar-ui.md#visual-weight) |
