...
- **`work_start`**: Requires the work item to have an `agent_role_id`. Generates a UUIDv7 session ID, transitions to `in_progress` and attaches the session ID via `Store.Start`, then creates the session and sends the kickoff via `WorkStartHandler` (in-process).
