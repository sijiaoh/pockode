...
チャットイベントは `chat.event` メソッドで送信され、`params.type` でイベント種別を区別する。

```json
{"jsonrpc":"2.0","method":"chat.event","params":{"type":"text","session_id":"...","content":"..."}}
```
...
| `done` | 応答完了 |
| `interrupted` | 中断完了 |
| `process_ended` | プロセス終了 |
| `permission_request` | 権限リクエスト |
| `request_cancelled` | リクエストキャンセル |
