...
- **The jump leaves the tail.** The list auto-follows new output while the user is at the bottom, and a jump is a deliberate move away from it; unless the jump clears the follow intent itself ([agent-chat.md](agent-chat.md#following-the-tail)), the next reflow of streaming output snaps back down and the tap appears to do nothing.
