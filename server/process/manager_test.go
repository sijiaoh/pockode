...
func TestManager_IdleReaper_SparesABackgroundWait(t *testing.T) {
	...
	m.reapIdleAsOf(overdue)
	if m.GetProcess("sess-1") != nil {
		t.Error("expected process to be reaped once the agent stopped waiting")
	}
}
...
func TestManager_Shutdown_WaitsForStreamingToFinish(t *testing.T) {
	...
	proc, _, err := m.GetOrCreateProcess(context.Background(), session.SessionMeta{ID: "sess-1", AgentType: session.AgentTypeClaude, Mode: session.ModeDefault})
	if err != nil {
...
// elapsed time, and answering starts the clock over.
//
...
func TestManager_IdleReaper_SparesAnUnansweredPrompt(t *testing.T) {
	...
	for name, prompt := range prompts {
		t.Run(name, func(t *testing.T) {
			...
			m.SetOnStateChange(recorder.record)
			...
			sess.emit(t, prompt)
			waitUntil(t, "the turn to pause on the prompt", proc.awaitingUserAnswer)
			...
			// Answering resumes the turn, and the clock starts from the answer:
			// the same instant that would have reaped an unprompted process
			...
			m.reapIdleAsOf(time.Now().Add(testIdleTimeout / 2))
			if m.GetProcess("sess-1") == nil {
				t.Fatal("process reaped right after the prompt was answered")
			}

			// And the exemption is gone with the prompt: once the answer goes
			// stale the process is reaped like any other.
			m.reapIdleAsOf(time.Now().Add(2 * testIdleTimeout))
			if m.GetProcess("sess-1") != nil {
...
// TestManager_IdleReaper_DoesNotSpareAPromptNobodyCanAnswer covers the two ways
// a prompt stops waiting on the user without being answered. Neither may leave
// the exemption behind: a process spared on a prompt that no longer exists is
// never reaped at all, since nothing left in the session will clear it.
func TestManager_IdleReaper_DoesNotSpareAPromptNobodyCanAnswer(t *testing.T) {
	// Withdrawal is the case the flag exists for. The turn keeps running through
	// it, so no idle follows to end the wait.
	endings := map[string]agent.AgentEvent{
		"the agent withdraws it": agent.RequestCancelledEvent{RequestID: "req-1"},
	...
	for name, ending := range endings {
		t.Run(name, func(t *testing.T) {
			...
			sess.emit(t, ending)
			waitUntil(t, "the prompt to stop counting as a wait", func() bool {
				return !proc.awaitingUserAnswer()
			})

			m.reapIdleAsOf(time.Now().Add(2 * testIdleTimeout))
			if m.GetProcess("sess-1") != nil {
				t.Error("expected process to be reaped once the prompt was gone")
			}
		})
	}
}
...
func TestManager_IdleReaper_SparesAPromptRaisedWhileAlreadyPaused(t *testing.T) {
	...
	sess.emit(t, agent.AskUserQuestionEvent{RequestID: "req-1", ToolUseID: "tool-1"})
	waitUntil(t, "the turn to pause on the first prompt", proc.awaitingUserAnswer)

	sess.emit(t, agent.RequestCancelledEvent{RequestID: "req-1"})
	waitUntil(t, "the first prompt to stop counting as a wait", func() bool {
		return !proc.awaitingUserAnswer()
	})

	sess.emit(t, agent.PermissionRequestEvent{RequestID: "req-2", ToolName: "Bash", ToolUseID: "tool-2"})
	waitUntil(t, "the turn to pause on the second prompt", proc.awaitingUserAnswer)
