...
// The point of MaxUploadSizeForBody is that it is quoted to a client as the
// ceiling for its own connection: it has to stay under the body limit it is
// given (with room for the multipart framing that limit counts and this one
// does not), and it must never claim more than the endpoint would store.
func TestMaxUploadSizeForBody(t *testing.T) {
	tests := []struct {
		name      string
		bodyLimit int64
		want      int64
	}{
		{"a route roomier than the endpoint is capped by the endpoint", MaxUploadSize * 4, MaxUploadSize},
		{"exactly the endpoint limit still leaves room for the framing", MaxUploadSize, MaxUploadSize - (1 << 20)},
		{"a narrow route decides the limit", 8 << 20, 7 << 20},
		{"a route too narrow for anything reports nothing, not a negative", 1 << 10, 0},
		{"no route at all", 0, 0},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := MaxUploadSizeForBody(tt.bodyLimit)
			if got != tt.want {
				t.Fatalf("MaxUploadSizeForBody(%d) = %d, want %d", tt.bodyLimit, got, tt.want)
			}
			if got > tt.bodyLimit {
				t.Fatalf("MaxUploadSizeForBody(%d) = %d, which does not fit the body it must travel in", tt.bodyLimit, got)
			}
			if got > MaxUploadSize {
				t.Fatalf("MaxUploadSizeForBody(%d) = %d, over what the endpoint would store (%d)", tt.bodyLimit, got, MaxUploadSize)
			}
		})
	}
}
