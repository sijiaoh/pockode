...
func (s *FileStore) CreateFork(ctx context.Context, sessionID string, fork ForkSpec) (SessionMeta, error) {
	...
	session := SessionMeta{
		...
		// The agent and its settings are inherited rather than taken from the
		// defaults: a conversation that continued under a different agent would not
		// be a fork of this one. Model and effort stay valid for free — they are
		// validated per agent, and the agent is the source's.
