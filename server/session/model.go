...
// to validate a choice and to answer `session.models` for the UI. An id from it
// reaches the CLI as `claude --model <id>`, or as the `model` field of Codex's
// `codex` tool call. Neither CLI can list its models (no subcommand does it on
// claude 2.1.263 / codex-cli 0.153.0), so this is maintained by hand and needs
// an update whenever an agent retires a model.
//
...
var agentModels = map[AgentType][]AgentOption{
	AgentTypeClaude: {
		{ID: "opus", Label: "Opus"},
...
func ModelsForAgent(agentType AgentType) []AgentOption {
...
func AllModels() map[AgentType][]AgentOption {
	return agentModels
}
...
func IsValidModel(agentType AgentType, model string) bool {
	return isOffered(agentModels, agentType, model)
}
