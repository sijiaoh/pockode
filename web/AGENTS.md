...
- **表单控件焦点**：有边框的文本框 / textarea / select 一律用 `ui/inputClass.ts` 的 `inputClass`（边框 + 焦点边框 + 光晕，用 `focus:`），padding、字号、背景、圆角留在调用处；按钮类控件仍用 `focus-visible:`。不要手写 `focus:border-*` —— 浅色主题下只变 1px 边框几乎看不出来
