...
function toUploadError(status: number, text: string): UploadError {
	...
	// The endpoint's own ceiling, quoted only to phrase this one refusal. What a
	// file is checked against beforehand is the connection's `max_upload_size`,
	// which on a relay is the smaller of the two (see docs/file.md#transfer).
...
export function uploadFile(request: UploadRequest): Promise<void> {
	...
	return new Promise((resolve, reject) => {
		...
		// said about it. Over a relay it is also how an envelope too large for
		// the tunnel surfaces, which is why the size is checked before sending.
		xhr.onerror = () =>
