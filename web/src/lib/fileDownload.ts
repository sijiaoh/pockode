 ...
 *
 * Downloads are chunked unconditionally. Over a relay tunnel the whole HTTP
 * response is read into memory and base64-encoded onto one shared WebSocket
 * connection (`server/relay/http.go`), which both inflates the body by 4/3 past
 * a 10 MiB envelope ceiling and holds the tunnel's write lock for the duration.
 * The browser cannot tell whether it reached the server directly or through a
 * relay, and chunking a local download costs one extra round trip per 4 MiB, so
...
export async function fetchFileBlob({
	...
	while (total === null || offset < total) {
		...
		// response is the very thing chunking exists to avoid — over a relay
		// tunnel it is read into memory, base64-inflated past the envelope
		// ceiling, and takes the shared connection down with it. This asks for a
